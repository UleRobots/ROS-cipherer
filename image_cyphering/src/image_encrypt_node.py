...
# We do not use cv_bridge beacuse it does not support CompressedImage in python.
...
class image_feature:
    ...
    #def pad_string(self, v_cadena):
        #new_str = v_cadena
        #pad_chars = INPUT_SIZE - (len(v_cadena) % INPUT_SIZE)
    ...
    def encrypt_chunk_AES(self, chunk,encryptor):
	if len(chunk) % 16 != 0:
	  chunk += ' ' * (16 - len(chunk) % 16)
	...
	encryption_mode = -1
	rospy.logerr('Cipher algorithm: %s', mode)
	if(mode in "AES"):
